...
class CasesController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $query              = Cases::with('hearings')->where('firm_id', Auth::guard('firm')->id());
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('fillings')) {
            ...
            $fillingsData = array_map(fn($fillingData) => [...$fillingData, 'case_id' => $case->id, 'firm_id' => Auth::guard('firm')->id()],$request->fillings);           
            CaseFillingDate::insert($fillingsData);
        }
        
        if ($request->has('hearings')) {
            
            $hearingsData = array_map(fn($hearingData) => [...$hearingData, 'case_id' => $case->id,  'firm_id' => Auth::guard('firm')->id()],$request->hearings);           
            CaseHearingDate::insert($hearingsData);
        }


        return redirect()
            ->route('firm.cases.index')
    ...
    public function show(string $id)
    {
        ...
        return view('firm.cases.show', compact('case'));
    }
    ...
    public function edit(string $id)
    {
        $case = Cases::where('id', $id)
            ->where('firm_id', Auth::guard('firm')->id())
            ->firstOrFail();
        return view('firm.cases.edit', compact('case'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $case = Cases::where('firm_id', Auth::guard('firm')->id())->findOrFail($id);
    ...
    public function destroy(Request $request, string $id)
    {
        ...
        Cases::where('firm_id', Auth::guard('firm')->id())->findOrFail($id)->delete();

        if ($request->ajax()) {
            return $this->successResponse('case deleted');
        }

        return redirect()->route('firm.cases.index')->with('success', 'Case deleted successfully.');
    }
    ...
    public function changeHearingDate(Request $request, $caseId, $hearingId)
    {
        ...
        $case    = Cases::where('firm_id', Auth::guard('firm')->id())->findOrFail($caseId);
