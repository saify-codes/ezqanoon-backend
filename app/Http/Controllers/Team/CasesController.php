...
class CasesController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $query              = Cases::with('hearings')->where($this->ownerForeignKey, $this->owner->id);
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('fillings')) {
            ...
            $fillingsData = array_map(fn($fillingData) => [...$fillingData, 'case_id' => $case->id, $this->ownerForeignKey => $this->owner->id],$request->fillings);           
            CaseFillingDate::insert($fillingsData);
        }
        
        if ($request->has('hearings')) {
            
            $hearingsData = array_map(fn($hearingData) => [...$hearingData, 'case_id' => $case->id,  $this->ownerForeignKey => $this->owner->id],$request->hearings);           
            CaseHearingDate::insert($hearingsData);
        }


        return redirect()
            ->route('team.cases.index')
    ...
    public function show(string $id)
    {
        ...
        return view('team.cases.show', compact('case'));
    }
    ...
    public function edit(string $id)
    {
        $case = Cases::where('id', $id)
            ->where($this->ownerForeignKey, $this->owner->id)
            ->firstOrFail();
        return view('team.cases.edit', compact('case'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $case = Cases::where($this->ownerForeignKey, $this->owner->id)->findOrFail($id);
    ...
    public function destroy(Request $request, string $id)
    {
        ...
        Cases::where($this->ownerForeignKey, $this->owner->id)->findOrFail($id)->delete();

        if ($request->ajax()) {
            return $this->successResponse('case deleted');
        }

        return redirect()->route('team.cases.index')->with('success', 'Case deleted successfully.');
    }
    ...
    public function changeHearingDate(Request $request, $caseId, $hearingId)
    {
        ...
        $case    = Cases::where($this->ownerForeignKey, $this->owner->id)->findOrFail($caseId);
