...
class ClientController extends Controller
{
    ...
    {
        if(!Auth::guard('team')->user()->hasPermission('clients:view')){
            abort(403);
        }
        ...
        if ($request->ajax()) {
            ...
            $query              =  Client::where($this->ownerForeignKey, $this->owner->id);
    ...
    {
        return view('team.client.create');
    }
    ...
    {
        if(!Auth::guard('team')->user()->hasPermission('clients:create')){
            abort(403);
        }
    ...
    {
        if(!Auth::guard('team')->user()->hasPermission('clients:edit')){
            abort(403);
        }

        $client = Client::where('id', $id)->where($this->ownerForeignKey, $this->owner->id)->firstOrFail();
        return view('team.client.edit', compact('client'));
    }
    ...
    public function show(string $id)
    {
        ...
        return view('team.client.show', compact('client'));
    }
    ...
    {
        if(!Auth::guard('team')->user()->hasPermission('clients:delete')){
            abort(403);
        }

        $case = Client::where($this->ownerForeignKey, $this->owner->id)->findOrFail($id);
        $case->delete();
        ...
        return redirect()->route('team.client.index')->with('success', 'Client deleted successfully.');
    }
}
