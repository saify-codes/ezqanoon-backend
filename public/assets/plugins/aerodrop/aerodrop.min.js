class AeroDrop {
    ...
    validateFile(file) {
        const { allowedFileTypes, fileSizeRules, maxFileSize, fallbackError } = this.options;
        ...
        // Validate file size based on rules
        if (Array.isArray(fileSizeRules)) {
            const rule = fileSizeRules.find(r => r.types.includes(file.type) && file.size > r.maxSize);
            if (rule) return rule.error ?? "File too big";
        }
    
        // Fallback size validation
        if (maxFileSize && file.size > maxFileSize) {
            return fallbackError || "File too big";
        }
    ...
    openCamera() {
        navigator.mediaDevices
            ...
            .then((stream) => {
                ...
                const captureBtn =
                    videoModal.querySelector(".capture-btn");
                const closeBtn = videoModal.querySelector(".close-btn");
