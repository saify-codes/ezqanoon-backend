        ...
        <ul class="navbar-nav">
            ...
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" href="#" id="profileDropdown" role="button"
                    ...
                    <img class="wd-30 ht-30 rounded-circle" src="{{ Auth::guard('firm')->user()->avatar }}" alt="profile">
                </a>
                <div class="dropdown-menu p-0" aria-labelledby="profileDropdown">
                    <div class="d-flex flex-column align-items-center border-bottom px-5 py-3">

                        <div class="mb-3">
                            <img class="wd-80 ht-80 rounded-circle" src="{{ Auth::guard('firm')->user()->avatar }}" alt="">
                        </div>

                        <span class="badge bg-primary mb-3">
                            {{ Auth::guard('firm')->user()->subscription->name }}
                        </span>

                        <div class="text-center">
                            <p class="tx-16 fw-bolder">{{Auth::guard('firm')->user()->name}}</p>
                            <p class="tx-12 text-muted">{{Auth::guard('firm')->user()->email}}</p>
                        </div>
                    </div>
                    <ul class="list-unstyled p-1">
                        <li class="dropdown-item py-2">
                            <a href="{{ route('firm.profile') }}" class="d-block text-body ms-0">
                        ...
                        <li class="dropdown-item py-2">
                            <a href="{{ route('firm.signout') }}" class="d-block text-body ms-0">
    ...
    <script>
        $(document).ready(function() {
            ...
            const searchableItems = [
                { text: 'Dashboard',        keywords: ['home', 'main', 'dashboard'], url: '{{ route("firm.dashboard") }}', icon: 'home' },
                { text: 'Clients',          keywords: ['clients', 'view clients', 'all clients', 'client list', 'manage clients'], url: '{{ route("firm.client.index") }}', icon: 'users' },
                { text: 'Add Client',       keywords: ['new client', 'create client', 'add client'], url: '{{ route("firm.client.create") }}', icon: 'user-plus' },
                { text: 'Cases',            keywords: ['cases', 'view cases', 'all cases', 'case list', 'manage cases'], url: '{{ route("firm.cases.index") }}', icon: 'briefcase' },
                { text: 'Add Case',         keywords: ['new case', 'create case', 'add case'], url: '{{ route("firm.cases.create") }}', icon: 'file-plus' },
                { text: 'Appointments',     keywords: ['appointments', 'view appointments', 'schedule'], url: '{{ route("firm.appointment.index") }}', icon: 'calendar' },
                { text: 'Profile',          keywords: ['my profile', 'account', 'profile settings'], url: '{{ route("firm.profile") }}', icon: 'user' },
            ];
