<x-firm.app>
    <div>
        <a href="{{route('firm.cases.create')}}" class="btn btn-primary btn-icon-text mb-3">
            ...
            $('#cases-table').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        data: function(row) {
                            // Build URLs
                            const showUrl   = `{{ route('firm.cases.show', ':id') }}`.replace(':id', row.id);
                            const editUrl   = `{{ route('firm.cases.edit', ':id') }}`.replace(':id', row.id);
                            const deleteUrl = `{{ route('firm.cases.destroy', ':id') }}`.replace(':id', row.id);
