<x-firm.app>
    <div>
        <a href="{{route('firm.client.create')}}" class="btn btn-primary btn-icon-text mb-3">
            ...
            $('#client-table').DataTable({
                ...
                columns: [
                    ...
                    { data: 'created_at' },
                    {
                        ...
                        data: function(row) {
                            // Build URLs
                            const showUrl   = `{{ route('firm.client.show', ':id') }}`.replace(':id', row.id);
                            const editUrl   = `{{ route('firm.client.edit', ':id') }}`.replace(':id', row.id);
                            const deleteUrl = `{{ route('firm.client.destroy', ':id') }}`.replace(':id', row.id);
