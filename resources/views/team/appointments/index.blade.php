    ...
    @push('custom-scripts')
        <script>
            $('#appointments-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('team.appointment.index') }}',
                columns: [{
                        data: 'id',
                    },
                    {
                   ...
                   
                    {
                        sortable: false,
                        data: (appointment) => {
                            const showUrl       = `{{ route('team.appointment.show', ':id') }}`.replace(':id',appointment.id);
                            const editUrl       = `{{ route('team.appointment.edit', ':id') }}`.replace(':id',appointment.id);
                            const deleteUrl     = `{{ route('team.appointment.destroy', ':id') }}`.replace(':id',appointment.id);
