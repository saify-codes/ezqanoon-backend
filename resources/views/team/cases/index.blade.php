<x-team.app>
    <div>
        <a href="{{route('team.cases.create')}}" class="btn btn-primary btn-icon-text mb-3">
            ...
            $('#cases-table').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        data: function(row) {
                            // Build URLs
                            const showUrl   = `{{ route('team.cases.show', ':id') }}`.replace(':id', row.id);
                            const editUrl   = `{{ route('team.cases.edit', ':id') }}`.replace(':id', row.id);
                            const deleteUrl = `{{ route('team.cases.destroy', ':id') }}`.replace(':id', row.id);
