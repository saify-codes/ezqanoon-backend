<x-team.app>
    <div>
        <a href="{{ route('team.invoice.create') }}" class="btn btn-primary btn-icon-text mb-3">
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(invoice) {
                            ...
                            if (invoice.type === 'MILESTONE') {
                                const showUrl = `{{ route('team.invoice.show', ':id') }}`.replace(':id', invoice.id);
                    ...
                    },
                    {
                        ...
                        data: function(invoice) {
                            // Build URLs
                            const showUrl   = `{{ route('team.invoice.show', ':id') }}`.replace(':id', invoice.id);
                            const editUrl   = `{{ route('team.invoice.edit', ':id') }}`.replace(':id', invoice.id);
                            const deleteUrl = `{{ route('team.invoice.destroy', ':id') }}`.replace(':id',invoice.id);
