<x-team.app>
    <div>
        <a href="{{route('team.client.create')}}" class="btn btn-primary btn-icon-text mb-3">
            ...
            $('#client-table').DataTable({
                ...
                columns: [
                    ...
                    { data: 'created_at' },
                    {
                        ...
                        data: function(row) {
                            // Build URLs
                            const showUrl   = `{{ route('team.client.show', ':id') }}`.replace(':id', row.id);
                            const editUrl   = `{{ route('team.client.edit', ':id') }}`.replace(':id', row.id);
                            const deleteUrl = `{{ route('team.client.destroy', ':id') }}`.replace(':id', row.id);
